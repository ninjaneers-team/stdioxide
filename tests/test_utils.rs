...
// `cat_cmd` uses `#[cfg]` item-level guards instead of `cfg_select!` because the two branches have
// different constness: the non-Windows branch can be `const fn`, the Windows one cannot. `cfg_select!`
// inside a single function body cannot express per-branch constness.
